...
public class SelectTest {
  ...
  public void testJoinWithWildcard() throws SQLException {
    ...
    writer.write(
        userToGateway.upsert(
            "user1", "gw2", Instant.now(), UserToGateway.newBuilder().setRole(Role.USER).build()),
        (succ, exc) -> {});
    ...
    try (Statement statement = calciteConnection.createStatement();
        ResultSet resultSet =
            statement.executeQuery(
                "select u.key, u.details.name, ug.key, ug.gateway, ug.role from proxima.\"USER\" as u "
      ...
      assertEquals("gw", resultSet.getString(4));
      assertEquals("OWNER", resultSet.getString(5));

      assertTrue(resultSet.next());
      assertEquals("user1", resultSet.getString(1));
      assertEquals("user", resultSet.getString(2));
      assertEquals("user1", resultSet.getString(3));
      assertEquals("gw2", resultSet.getString(4));
      assertEquals("USER", resultSet.getString(5));
    }
  }

  @Test
  public void testSelectInClause() throws SQLException {
    SchemaPlus proxima = calciteConnection.getRootSchema().getSubSchema("PROXIMA");
    EntityTable gateway = (EntityTable) proxima.getTable("GATEWAY");
    Repository repo = gateway.getRepo();
    DirectDataOperator direct = gateway.getDirect();
    ...
    OnlineAttributeWriter writer = Optionals.get(direct.getWriter(userToGateway));
    writer.write(
        userToGateway.upsert(
            "user1", "gw", Instant.now(), UserToGateway.newBuilder().setRole(Role.OWNER).build()),
        (succ, exc) -> {});
    writer.write(
        userToGateway.upsert(
            "user2", "gw2", Instant.now(), UserToGateway.newBuilder().setRole(Role.USER).build()),
        (succ, exc) -> {});

    try (Statement statement = calciteConnection.createStatement();
        ResultSet resultSet =
            statement.executeQuery(
                "select key from proxima.\"USER.GATEWAY\" where key in ('user1', 'user2')")) {

      List<String> keys = new ArrayList<>();
      while (resultSet.next()) {
        keys.add(resultSet.getString("key"));
      }
      assertEquals(List.of("user1", "user2"), keys);
    }
  }
  ...
  public void testSelectArray() throws SQLException {
    ...
    writer.write(
        details.upsert(
            "gw",
            Instant.now(),
            GatewayDetails.newBuilder().setName("gw").addTags("t1").addTags("t2").build()),
        (succ, exc) -> {});

    try (Statement statement = calciteConnection.createStatement();
        ResultSet resultSet =
            statement.executeQuery(
                "select g.details.tags from proxima.gateway as g where key = 'gw'")) {

      assertTrue(resultSet.next());
      Object array = resultSet.getObject(1);
      assertTrue(array instanceof java.sql.Array || array instanceof List);
      if (array instanceof java.sql.Array) {
        Object[] arr = (Object[]) ((java.sql.Array) array).getArray();
        assertEquals("t1", arr[0]);
        assertEquals("t2", arr[1]);
      } else {
        List<?> list = (List<?>) array;
        assertEquals(List.of("t1", "t2"), list);
      }
    }
  }
}
