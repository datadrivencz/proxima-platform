...
public class EntityTable extends AbstractTable
  ...
  private final @Nullable RandomAccessReader listKeysReader;
  ...
  public Enumerable<Object[]> scan(DataContext root) {
    return scanKeys(null, null);
  }

  @Override
  public Enumerable<Object[]> scan(DataContext root, List<RexNode> filters) {
    return scanKeys(extractKeysFromFilters(filters), null);
  }

  @Override
  public Enumerable<Object[]> scan(DataContext root, List<RexNode> filters, int[] projects) {
    return scanKeys(extractKeysFromFilters(filters), projects);
  }
  ...
  private Enumerable<Object[]> scanKeys(@Nullable List<String> keys, int @Nullable [] projects) {
    Deque<String> entities = new LinkedList<>();
    if (keys == null) {
      Preconditions.checkArgument(listKeysReader != null, "Cannot scan keys of entity %s", entity);
      ...
      public Enumerator<Object[]> enumerator() {
        return new Enumerator<>() {
          ...
          public Object[] current() {
            int resultSize = projects == null ? attributes.size() + 1 : projects.length;
            Object[] res = new Object[resultSize];
            for (int i = 0; i < resultSize; i++) {
              int projIndex = projects == null ? i : projects[i];
              if (projIndex == 0) {
                res[i] = currentKey;
              } else {
                AttributeDescriptor<?> a = attributes.get(projIndex - 1);
                Optional<? extends KeyValue<?>> kv = reader.get(currentKey, a);
                if (kv.isPresent()) {
                  res[i] = TypeUtil.convertKv(kv.get());
                } else {
                  res[i] = null;
                }
              }
