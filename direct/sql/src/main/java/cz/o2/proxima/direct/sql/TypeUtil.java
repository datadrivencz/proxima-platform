...
class TypeUtil {
  ...
  private static Object convert(SchemaTypeDescriptor<?> schema, Object value) {
    ...
    if (schema.isStructureType()) {
      ...
      Map<String, SchemaTypeDescriptor<?>> fields = structDesc.getFields();
      Object[] res = new Object[fields.size()];
      int pos = 0;
      for (Map.Entry<String, SchemaTypeDescriptor<?>> e : fields.entrySet()) {
    ...
    if (schema.isArrayType()) {
      ...
      SchemaTypeDescriptor<?> valueDesc = arrayDesc.getValueDescriptor();
      List<?> list = (List<?>) value;
      return list.stream().map(v -> convert(valueDesc, v)).collect(Collectors.toList());
    }
    return value;
  }
}
