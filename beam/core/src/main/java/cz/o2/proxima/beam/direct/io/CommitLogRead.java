...
public class CommitLogRead extends PTransform<PBegin, PCollection<StreamElement>> {
  ...
  private static class OffsetWatermarkEstimator implements WatermarkEstimator<Instant> {

    private final OffsetRange restriction;
    private Instant watermark;

    public OffsetWatermarkEstimator(OffsetRange restriction, Instant initialWatermark) {
      this.restriction = restriction;
      this.watermark = Objects.requireNonNull(initialWatermark);
    }

    @Override
    public @Nonnull Instant currentWatermark() {
      long newWatermark;
      if (restriction.getEndOffsetInclusive() != null) {
        newWatermark = restriction.getEndOffsetInclusive().getWatermark();
      } else if (restriction.getStartOffset() != null) {
        newWatermark = restriction.getStartOffset().getWatermark();
      } else {
        newWatermark = Watermarks.MIN_WATERMARK;
      }
      if (newWatermark > watermark.getMillis()) {
        watermark = Instant.ofEpochMilli(newWatermark);
      }
      System.err.println(
          " *** watermark of restriction " + restriction + " calculated as " + watermark);
      return watermark;
    }
